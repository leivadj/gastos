...
function IconoFlecha({ className = "shrink-0 text-gray-300" }: { className?: string }) {
  return (
...
export default function PresupuestoPage() {
  ...
  return (
    <div className="space-y-4 pb-10">
      <div>
        <h1 className="text-lg font-bold text-gray-800">Presupuesto</h1>
        <p className="text-xs capitalize text-gray-400">{nombreMes()} · cómo se reparte cada peso.</p>
      </div>
            ...
            <div className="flex-1 space-y-2 text-sm">
              {legendPrincipal.map((d, i) => (
                <div key={d.name} className="flex items-center justify-between gap-2">
                  ...
                  <span className="font-medium text-gray-800">{pct(d.value)}</span>
                </div>
              ))}
              {restoCategorias.length > 0 && (
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="font-medium text-gray-400">{pct(restoTotal)}</span>
                </div>
              )}
            </div>
          </div>
      ...
      {totalGastos > 0 && (
        <Card>
          ...
          <div className="h-3.5 w-full overflow-hidden rounded-full bg-pink-100">
        ...
        <Link href="/servicios-basicos">
          <Card className="active:bg-gray-50">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold text-gray-600">Fijos obligatorios de monto variable</p>
              <IconoFlecha />
            </div>
            <p className="mb-3 mt-0.5 text-[11px] text-gray-400">
              ...
              {gastosVariables.map((g) => {
                ...
                return (
                  <li key={g.id} className="flex items-center justify-between py-2 text-sm">
                    <div>
                      <p className="font-medium text-gray-700">{g.descripcion}</p>
        ...
        <Link href="/metas-ahorro">
          <Card className="active:bg-gray-50">
            <div className="mb-3 flex items-center justify-between">
              <p className="text-sm font-semibold text-gray-600">Metas de ahorro</p>
              <IconoFlecha />
            </div>
