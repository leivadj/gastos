              ...
              <button type="button" onClick={onClick} className="flex w-full items-start gap-3 text-left">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <p className="truncate font-semibold text-gray-800 dark:text-white">{m.nombre}</p>
                    {cumplida && (
                      <span className="shrink-0 rounded-full bg-emerald-50 dark:bg-emerald-900/40 px-2 py-0.5 text-[10px] font-bold text-emerald-500 dark:text-emerald-300">
                        Cumplida
                      </span>
                    )}
                  </div>
            ...
            <div>
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-start gap-3">
                  <EntidadAvatar entidad={null} icono={m.icono} nombreFallback={m.nombre} className="h-9 w-9" />
                  <div>
                    <p className="pt-1 font-semibold text-gray-800 dark:text-white">{m.nombre}</p>
                    <p className="text-xs text-gray-400 dark:text-gray-500">
                      {formatCLP(m.monto_actual)} de {formatCLP(m.monto_objetivo)}
                    </p>
                  </div>
              ...
              <div className="mt-3 space-y-1.5">
                <p className="text-[11px] font-bold uppercase tracking-wide text-gray-400 dark:text-gray-500">Aportes</p>
                {aportesMeta.length === 0 ? (
                  <p className="text-xs text-gray-400 dark:text-gray-500">Todavía no hay aportes registrados.</p>
                ) : (
                    ...
                    {aportesMeta.map((a) => (
                      <li key={a.id} className="flex items-center justify-between gap-2 text-xs">
                        ...
                        <span className="flex items-center gap-2">
                          ...
                          <button onClick={() => borrarAporte(a.id)} className="text-gray-300 dark:text-gray-600 hover:text-red-400">
                            ✕
                          </button>
                        </span>
                      </li>
              ...
              {aportandoId === m.meta_id ? (
                <div className="mt-3 space-y-2 rounded-lg bg-purple-50 dark:bg-white/10 p-3">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-gray-200 dark:border-white/10 dark:bg-white/5 dark:text-white px-3 py-1.5 text-sm"
                  />
                  <p className="text-[10.5px] text-gray-400 dark:text-gray-500">Usa un monto negativo para registrar un retiro.</p>
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="flex-1 rounded-lg bg-white dark:bg-gray-800 py-1.5 text-xs font-semibold text-gray-400 dark:text-gray-500"
                    >
                      Cancelar
                    </button>
                  </div>
              ...
              <div className="mt-3 flex gap-2">
                <button
                  ...
                  className="flex-1 rounded-lg bg-gray-50 dark:bg-white/5 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400"
                >
                  Editar
                </button>
                <button
  ...
  return (
    <div className="space-y-4 pb-10">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-gray-800 dark:text-white">Metas de ahorro</h1>
          <p className="text-xs text-gray-400 dark:text-gray-500">Objetivos puntuales, no afectan tu disponible del mes.</p>
        </div>
        <button
        ...
        <Card>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-gray-500 dark:text-gray-400">Monto objetivo</label>
                <input
                  required
                  type="number"
                  min={0}
              ...
              <div>
                <label className="text-xs text-gray-500 dark:text-gray-400">Fecha objetivo (opcional)</label>
                <input
                  type="date"
                  value={fechaObjetivo}
      ...
      {enProgreso.length > 0 && (
        <div className="space-y-3">
          <p className="px-1 text-xs font-bold uppercase tracking-wide text-gray-400 dark:text-gray-500">En progreso</p>
      ...
      {cumplidas.length > 0 && (
        <div className="space-y-3">
          <p className="px-1 text-xs font-bold uppercase tracking-wide text-gray-400 dark:text-gray-500">Cumplidas</p>
      ...
      {metas.length === 0 && !mostrarForm && (
        <p className="text-center text-sm text-gray-400 dark:text-gray-500">Todavía no tienes metas de ahorro.</p>
      )}
